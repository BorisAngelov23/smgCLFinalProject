...
class TeamViewsTestCase(TestCase):
    ...
    def test_team_register_view_post(self):
        ...
        response = self.client.post(
            reverse("team_register"), data={"paralelki": ["A", "B", "C"]}
        )
        self.assertEqual(response.status_code, 302)
...
class TeamRegistrationFormTest(TestCase):
    ...
    def test_invalid_form(self):
        ...
        self.assertIn("You can't choose more than 2 classes",
...
class PlayerFormTest(TestCase):
    ...
    def test_invalid_form(self):
        ...
        form = PlayerForm(data=form_data, user=user)
        self.assertFalse(form.is_valid())
        self.assertIn("Name should only contain letters.",
                      form.errors["__all__"][0])


class PlayerModelTest(TestCase):
    def setUp(self):
        captain_user = CaptainUser.objects.create_user(
            username="captain", password="testpassword"
        )
        team = Team.objects.create(
            captain=captain_user, name="Team A", grade="10")
    ...
    def test_player_creation(self):
        ...
        self.assertEqual(
            str(player), f"{player.first_name} {player.last_name}")
...
class TeamModelTest(TestCase):
    def setUp(self):
        ...
        self.team = Team.objects.create(
            captain=captain_user, name="Team A", grade="10", paralelki=["A", "B", "C"]
        )
